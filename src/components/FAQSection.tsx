...
import faqBg from "@/assets/section-faq-bg.jpg";
...
const FAQSection = ({ faqs, title = "Veelgestelde vragen", className = "" }: FAQSectionProps) => (
  <section className={`relative overflow-hidden ${className}`}>
    <img src={faqBg} alt="" className="absolute inset-0 w-full h-full object-cover" loading="lazy" />
    ...
    <div className="container relative py-14 sm:py-20 md:py-28 px-4 sm:px-6">
      ...
      <div className="max-w-2xl mx-auto space-y-3 sm:space-y-4">
        {faqs.map((faq) => (
          <details key={faq.q} className="group bg-card/80 backdrop-blur-sm rounded-xl border border-border shadow-sm">
