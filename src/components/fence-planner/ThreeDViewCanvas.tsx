...
const SCALE = 0.01; // 1cm = 0.01 units (so 180cm = 1.8 units)
...
/* Panel face with pattern texture */
function Panel({
  x,
  width,
  height,
  ...
  // For horizontal planks, render multiple thin boxes
  ...
  return (
    <div className="w-full h-full min-h-[400px] relative" style={{ height: "100%" }}>
      ...
      <div className="absolute bottom-4 right-4 bg-background/70 backdrop-blur-sm px-3 py-1.5 rounded-md border border-border">
        <p className="text-xs text-muted-foreground">🖱️ Sleep om te draaien · Scroll om te zoomen</p>
      </div>
    </div>
  );
};
